...
package metaheuristics.NormalizeMOEAD;
...
import util.Comparator.DominationComparator;
import util.Comparator.MOEADComparator.NormalizeMOEAComparator;
import util.ScalarzingFunction.ScalarzingFunction;
import util.ScalarzingFunction.ScalarzingFunctionFactory;



public class NormilizeMOEAD extends Algorithm {
	 ...
	 *            Problem to solve
	 */
	public NormilizeMOEAD(Problem problem) {
		super(problem);
	} // DMOEA
...
//		 NormalizationWithNadia();
	...
	//更新されたらtrue そうでないならfalse
	public boolean UpdataNonDominatedSolution(Solution newSol) throws JMException{
		if(nonDominatedSols.size() == 0){
			nonDominatedSols.add(newSol);
			UpdataMax(newSol);
			UpdataMin(newSol);
			return true;
		}
		...
		//judge the new Solution is the nonDominatedsol
		for(int solnum = 0 ; solnum < nonDominatedSols.size();solnum++){
			Solution sol = nonDominatedSols.get(solnum);
			//重複個体をなくすため
			if(dominationComparator.execute(sol, newSol) != -1){
				flag = true;
				break;
			}
		}
		...
		for(int solnum = 0 ; solnum < nonDominatedSols.size();solnum++){
			Solution sol = nonDominatedSols.get(solnum);
			if(dominationComparator.execute(newSol, sol) == 1){
				nonDominatedSols.remove(solnum--);
			}
		}
	...
	public void initPopulation() throws JMException, ClassNotFoundException {
		...
		for (int i = 0; i < populationSize_; i++) {
			...
			evaluations_++;
			UpdataNonDominatedSolution(newSolution);
